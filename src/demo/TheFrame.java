...
public class TheFrame extends JFrame {
	...
	public TheFrame() {
		...
		mainCfg = new SLConfig(panel).gap(10, 10).row(1f).col(250).col(1f)
				.col(2f).beginGrid(0, 0).row(2f).row(1f).col(1f)
				.place(0, 0, p1).place(1, 0, p2).endGrid().beginGrid(0, 1)
				.row(1f).row(2f).col(1f).place(0, 0, p3).place(1, 0, p4)
				.endGrid().place(0, 2, p5);

		p1Cfg = new SLConfig(panel).gap(10, 10).row(1f).col(250).col(1f)
				.col(2f).place(0, 0, p1).beginGrid(0, 1).row(1f).row(2f)
				.col(1f).place(0, 0, p3).place(1, 0, p4).endGrid()
				.place(0, 2, p5);

		p2Cfg = new SLConfig(panel).gap(10, 10).row(1f).col(2f).col(1f)
				.beginGrid(0, 0).row(2f).row(1f).col(1f).beginGrid(0, 0)
				.row(1f).col(250).col(1f).place(0, 0, p1).place(0, 1, p3)
				.endGrid().place(1, 0, p2).endGrid().place(0, 1, p5);

		p3Cfg = new SLConfig(panel).gap(10, 10).row(1f).col(2f).col(1f)
				.place(0, 0, p3).place(0, 1, p5);

		p4Cfg = new SLConfig(panel).gap(10, 10).row(1f).row(1f).col(1f).col(1f)
				.place(0, 0, p1).place(1, 0, p2).place(0, 1, p3)
				.place(1, 1, p4);
	...
	private final Runnable p1Action = new Runnable() {
		...
		public void run() {
			...
			panel.createTransition()
					.push(new SLKeyframe(p1Cfg, 0.6f).setEndSide(SLSide.BOTTOM,
							p2).setCallback(new SLKeyframe.Callback() {
						@Override
						public void done() {
	...
	private final Runnable p1BackAction = new Runnable() {
		...
		public void run() {
			...
			panel.createTransition()
					.push(new SLKeyframe(mainCfg, 0.6f).setStartSide(
							SLSide.BOTTOM, p2).setCallback(
		...
		public void run() {
			...
			panel.createTransition()
					.push(new SLKeyframe(p2Cfg, 0.6f)
							.setEndSide(SLSide.BOTTOM, p4).setDelay(0.6f, p2)
		...
		public void run() {
			...
			panel.createTransition()
					.push(new SLKeyframe(mainCfg, 0.6f)
							.setStartSide(SLSide.BOTTOM, p4).setDelay(0.6f, p4)
							.setCallback(new SLKeyframe.Callback() {
								@Override
								public void done() {
									p2.setAction(p2Action);
	...
	private final Runnable p4Action = new Runnable() {
		...
		public void run() {
			...
			panel.createTransition()
					.push(new SLKeyframe(p4Cfg, 0.6f).setEndSide(SLSide.RIGHT,
							p5).setCallback(new SLKeyframe.Callback() {
						@Override
						public void done() {
	...
	private final Runnable p5Action = new Runnable() {
		...
		public void run() {
			...
			panel.createTransition()
					.push(new SLKeyframe(p5Cfg, 0.8f).setEndSide(SLSide.LEFT,
							p1, p2, p3, p4).setCallback(
							new SLKeyframe.Callback() {
								@Override
								public void done() {
	...
	private final Runnable p5BackAction = new Runnable() {
		...
		public void run() {
			...
			panel.createTransition()
					.push(new SLKeyframe(mainCfg, 0.8f).setStartSide(
							SLSide.LEFT, p1, p2, p3, p4).setCallback(
							new SLKeyframe.Callback() {
								@Override
								public void done() {
									p5.setAction(p5Action);
